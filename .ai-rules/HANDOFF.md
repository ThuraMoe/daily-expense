...
**Active task:** TASK-002 - App Shell & Layout

**What was done last session:**
- Archived the old implementation into `src/legacy/` instead of deleting it
- Created a minimal active rewrite baseline and fixed broken bootstrap imports in `src/main.tsx`
- Verified Tailwind CSS, Firebase config shape, Vite build, Docker build, and port alignment on `43177`
- Fixed Docker runtime mismatch by aligning `Dockerfile` with the actual dev command and removed the obsolete Compose `version` field
- Reviewed and removed unused legacy dependencies from `package.json`, refreshed `package-lock.json`, and confirmed the app still builds

**What to do next session:**
- Start **TASK-002** (App Shell & Layout)
- Agree navigation style with human before building the layout
- Build the new layout shell in active `src/` only, keeping `src/legacy/` as reference-only archive
